...
def construct_waltz():
    ...
    print("The complete sets of minuets that are playing are: ")
    for i in minuet_table:
        randomize = minuet_filename(i[roll_dice(2)])
        minuets.append(randomize)
        minuetList += ([i[roll_dice(2)]])

    for i in minuetList:
        minuetList[j] = minuet_filename(i)
        j += 1

    print(str(minuetList))

    print()    
    print("The complete sets of trios that are playing are: ")
    for i in trio_table:
        randomize = trio_filename(i[roll_dice()])
        trios.append(randomize)
        trioList += ([i[roll_dice()]])

    for i in trioList:
        trioList[k] = trio_filename(i)
        k += 1

    print(str(trioList))

    print()
    print("The entire set of minuets and trios that are playing are: ")
    for i in (minuets + trios):
        complete_wav_object = simpleaudio.WaveObject.from_wave_file(i)
        completeList.append(complete_wav_object)

    combinedList = minuetList + trioList
    print(combinedList)
